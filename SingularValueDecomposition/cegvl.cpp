...
int count = 0;
  do {
   ...
   ShiftMatrix(seg, shift * minusone);
...
exit(1);
 ...
 sort(eigvls_.begin(), eigvls_.end(), cmore());
 reigvls_.reserve(eigvls_.size());
 for(int i = 0; i < eigvls_.size(); ++i) {
  reigvls_.push_back(eigvls_[i].real());
 }
